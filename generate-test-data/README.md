...
The command line syntax is the same as above. For example, to produce an example of JER with a variable length bitstring:

```bash
echo "<VehicleEventFlags>01110001001111</VehicleEventFlags>" > vef.xml
./converter-new-jer -p VehicleEventFlags -ixer -ojer vef.xml
```

which outputs:
```json
{"value":"713C","length":14}
```
